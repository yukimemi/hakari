...
| からだ | 全身写真 → 骨格を実測 → 体型を 3D 表示。スライダーで目標体重の姿へモーフ。推定ウエストと推定体脂肪率の推移グラフ |
...
- **生成は持ち主のみ** (`api/clip.ts` がサーバ側で判定、rules でも二重に)。
  招待した人は採用済みのクリップを見るだけ。
- 生成 → 試写 → 「これにする」で確定。押すまで保存されない。
- Google 側の動画は**2 日で消える**ので、確定時に Storage の `clips/` へ
  コピーし、`config/clips` にどれを採用したかを書く。
- 1 日の生成本数は `DAILY_CLIP_LIMIT` (既定 10) で別枠管理。テキストの
  `DAILY_CALL_LIMIT` とは別のカウンタ。
...
src/
  ...
  vision/measure.ts  MediaPipe で骨格を実測 (ウエストは AI が写真から判断)
